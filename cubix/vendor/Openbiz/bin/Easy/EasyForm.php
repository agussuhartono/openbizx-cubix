...
class EasyForm extends MetaObject implements Statefullable
{
    ...
    public function getDataObj()
    {
        if (!$this->dataObj) {
            if ($this->dataObjName) {
                ...
                $this->dataObj = Openbiz::getObject($this->dataObjName);
            }
            if ($this->dataObj) {
                //DebugLine::show(__METHOD__.__LINE__);
    ...
    public function loadPicker($formName, $elementName = "")
    {
        ...
        $pickerForm = Openbiz::getObject($formName);
        ...
        Openbiz::$app->getClientProxy()->redrawForm("DIALOG", $pickerForm->render());
    }
    ...
    {
        $service = Openbiz::getService($class);
        if ($param) {
    ...
    {
        Openbiz::$app->getClientProxy()->closePopup();
    }
    ...
    {
        $page = Openbiz::$app->getClientProxy()->getFormInputs(str_replace(".", "_", $this->objectName) . '_' . $elemName);
        $this->gotoPage($page);
    }

    public function setPageSize($elemName)
    {
        $pagesize = Openbiz::$app->getClientProxy()->getFormInputs(str_replace(".", "_", $this->objectName) . '_' . $elemName);
        $this->range = $pagesize;
        $this->UpdateForm();
    }
    ...
    public function copyRecord($id = null)
    {
        ...
        if (!$id) {
            Openbiz::$app->getClientProxy()->showClientAlert($this->getMessage("PLEASE_EDIT_A_RECORD"));
            return;
        }
        ...
        if ($formName == null) {
            if ($this->referenceFormName == null) {
                ...
                if ($formName = $this->referenceFormName) {
                    ...
                    $formObj = Openbiz::getObject($this->referenceFormName);
        ...
        //if($this->getWebpageObject()->isInFormRefLibs($formName))
        {
            ...
            if (!$formObj->canDisplayForm()) {
                ...
                if (strtoupper($this->formType) == "LIST") {
                    Openbiz::$app->getLog()->log(LOG_ERR, "DATAOBJ", "DataObj error = " . $errorMsg);
                    Openbiz::$app->getClientProxy()->showClientAlert($formObj->errorMessage);
                } else {
            ...
            switch ($target) {
                ...
                case "Dialog":
                    ...
                    break;
                default:
                    Openbiz::$app->getClientProxy()->redrawForm($this->objectName, $formObj->render());
            }
        }
    }
    ...
    public function autoSuggest($input)
    {
        ...
        $value = Openbiz::$app->getClientProxy()->getFormInputs($input);
...
$(jq('" . $this->objectName . "'))[0].oncontextmenu=function(event){return Openbiz.Menu.show(event, '" . $this->objectName . "_contextmenu');};
$(jq('" . $this->objectName . "')).bind('click',Openbiz.Menu.hide);
</script>";
        } else {
            $str .= "
<script>
$('" . $this->objectName . "').removeAttribute('onContextMenu');
$('" . $this->objectName . "').oncontextmenu=function(event){return Openbiz.Menu.show(event, '" . $this->objectName . "_contextmenu');};
$('" . $this->objectName . "').observe('click',Openbiz.Menu.hide);
</script>";
        }
        return $str;
    }
    ...
    {
        if ($redrawForm) {
            Openbiz::$app->getClientProxy()->redrawForm($this->objectName, $this->renderHTML());
        }
    ...
    protected function getInvokingElement()
    {
        ...
        // __this is elementName:eventHandlerName
        $elementAndEventName = Openbiz::$app->getClientProxy()->getFormInputs("__this");
    ...
    protected function setClientScripts()
    {
        ...
        if ($this->jsClass != "Openbiz.Form" && $this->jsClass != "Openbiz.TableForm" && $this->jsClass != "")
            Openbiz::$app->getClientProxy()->appendScripts($this->jsClass, $this->jsClass . ".js");
        /*
          if ($this->formType == 'LIST')
          {
          Openbiz::$app->getClientProxy()->appendScripts("tablekit", "tablekit.js");
          } */
    }
